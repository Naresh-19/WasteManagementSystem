...
class add_stock(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        print("Request Data:", request.POST)  # Print request data to inspect
        try:
            ...
            try:
                ...
                raise Http404("Product does not exist")
            # Check if stock history exists for the product
            try:
                stock = stock_his.objects.get(product=pd)
            except stock_his.DoesNotExist:
                # If stock history does not exist, create a new entry
                stock = stock_his.objects.create(product=pd, stock=0)
            # Update stock
            stock.stock += int(qty)
            stock.save()
            message = 'Stock Added'
